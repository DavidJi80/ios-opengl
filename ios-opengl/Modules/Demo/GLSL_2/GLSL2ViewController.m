...
@implementation GLSL2ViewController
...
- (void)compileShaders {
    // 生成一个顶点着色器对象
    GLuint vertexShader = [self compileShader:@"Glsl2Vertex" withType:GL_VERTEX_SHADER];
    // 生成一个片段着色器对象
    GLuint fragmentShader = [self compileShader:@"Glsl2Fragment" withType:GL_FRAGMENT_SHADER];
    ...
    // 在着色器编辑和连接之后，需要获取uniform的位置值，如果glGetUniformLocation返回-1就代表没有找到这个位置值。
    ...
    glUniform4f(vertexColorLoc, 0.7f, 1.0f, 0.1f, 1.0f);
